"""Pilot visual-object HOT3D-Clips baseline training.

This script validates cached observation-frame visual features with object
metadata. It uses derived proxy labels and must not be reported as a final
paper result.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train visual-object HOT3D-Clips pilot baseline.")
...
def make_loader(dataset: HOT3DClipsDataset, batch_size: int, shuffle: bool, num_workers: int) -> DataLoader:
    return DataLoader(
        ...
        collate_fn=collate_hot3d_visual_object_batch,
    )


def collate_hot3d_visual_object_batch(items: list[dict[str, Any]]) -> dict[str, Any]:
    tensor_keys = (
        "features",
        "frame_features",
        "visual_features",
...
def lightweight_dataset_summary(dataset: HOT3DClipsDataset, name: str) -> dict[str, Any]:
    return {
        ...
        "visual_feature_cache": visual_cache_summary(dataset),
    }


def visual_cache_summary(dataset: HOT3DClipsDataset) -> dict[str, Any]:
...
def verify_visual_alignment(dataset: HOT3DClipsDataset, split_name: str) -> dict[str, Any]:
    ...
    if missing:
        raise RuntimeError(f"{split_name}: missing cached visual features for {len(missing)} samples.")
...
def load_previous_object_aware_metrics(path: str | Path) -> dict[str, Any] | None:
...
def comparison_summary(current: dict[str, float], previous: dict[str, Any] | None) -> dict[str, Any]:
    if previous is None:
        return {
            "available": False,
            "note": "Previous object-aware pilot metrics file not found.",
        }
    ...
    for key in keys:
        if key in current and key in previous:
            ...
            comparison[key] = {
                "object_aware_previous": previous_value,
                "visual_object_current": current_value,
...
def main() -> None:
    ...
    print("Using cached image_stats visual features from observation frames only.")

    allow_forecast_object_input = bool(config.get("allow_forecast_object_input", False))
    if allow_forecast_object_input:
        raise ValueError("Visual-object pilot refuses allow_forecast_object_input=true.")
    ...
    previous_metrics = load_previous_object_aware_metrics(
        config.get("object_aware_metrics_path", "results/logs/hot3d_object_aware_baseline_pilot.json")
    )
    comparison = comparison_summary(test_metrics, previous_metrics)
    metrics_payload = {
        "notice": PILOT_NOTICE,
        ...
        "history": history,
        "test": test_metrics,
        "pilot_comparison_with_object_aware": comparison,
    }

    metrics_path = Path(config.get("metrics_path", "results/logs/hot3d_visual_object_baseline_pilot.json"))
    checkpoint_path = Path(config.get("checkpoint_path", "results/checkpoints/hot3d_visual_object_baseline_pilot.pt"))
    ...
    print("pilot_comparison_with_object_aware=" + json.dumps(comparison, sort_keys=True))
