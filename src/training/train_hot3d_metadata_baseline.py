"""Pilot metadata-only HOT3D-Clips baseline training.

This script is for pipeline validation only. It uses derived proxy labels and
must not be reported as a final paper result.
"""
...
from src.models.hot3d_metadata_baseline import HOT3DMetadataBaseline
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train metadata-only HOT3D-Clips pilot baseline.")
...
def make_loader(dataset: HOT3DClipsDataset, batch_size: int, shuffle: bool, num_workers: int) -> DataLoader:
    return DataLoader(
        ...
        collate_fn=collate_hot3d_metadata_batch,
    )


def collate_hot3d_metadata_batch(items: list[dict[str, Any]]) -> dict[str, Any]:
    tensor_keys = (
        "features",
        "frame_features",
        "future_hand_pose",
    ...
    for key in ("target_object_name", "proxy_label_status", "clip_id", "sample_id", "metadata"):
        batch[key] = [item[key] for item in items]
    if "frames" in items[0]:
        batch["frames"] = torch.stack([item["frames"] for item in items], dim=0)
    return batch
...
def compute_loss(
    ...
    class_loss = torch.nn.functional.cross_entropy(outputs["object_logits"], batch["target_object_label"])
...
def evaluate(
    ...
    for batch in loader:
        ...
        labels = batch["target_object_label"]
...
def main() -> None:
    ...
    print("No image data is used in this metadata-only pilot baseline.")

    label_map_path = config.get("label_map_path", "data/processed/hot3d_target_object_label_map.json")
    train_dataset = HOT3DClipsDataset(config["train_index"], mode="metadata_only", label_map_path=label_map_path)
    val_dataset = HOT3DClipsDataset(config["val_index"], mode="metadata_only", label_map_path=label_map_path)
    test_dataset = HOT3DClipsDataset(config["test_index"], mode="metadata_only", label_map_path=label_map_path)
    ...
    metrics_path = Path(config.get("metrics_path", "results/logs/hot3d_metadata_baseline_pilot.json"))
    checkpoint_path = Path(config.get("checkpoint_path", "results/checkpoints/hot3d_metadata_baseline_pilot.pt"))
