"""Vision-language candidate-ranking pilot model for HOT3D-Clips."""
...
class HOT3DVLCandidateRanker(nn.Module):
    """Score object candidates using hand metadata, geometry, and frozen text features."""
    ...
    def __init__(
        ...
        per_candidate_dim = hidden_dim + candidate_hidden_dim + text_hidden_dim
        self.rank_head = nn.Sequential(
            nn.Linear(per_candidate_dim, fusion_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(fusion_hidden_dim, 1),
        )
        self.pose_head = nn.Sequential(
            nn.Linear(hidden_dim + candidate_hidden_dim + text_hidden_dim, fusion_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        text_context = self.text_encoder(candidate_text_features)

        repeated_temporal = temporal_context.unsqueeze(1).expand(-1, candidate_context.shape[1], -1)
        rank_features = torch.cat([repeated_temporal, candidate_context, text_context], dim=-1)
        ...
        pooled_text = self._masked_mean(text_context, candidate_mask)
        pose_features = torch.cat([temporal_context, pooled_candidates, pooled_text], dim=-1)
        return {
